...
int main(void) {
  ...
  char16_t *utf16_output = (char16_t *)malloc(expected_utf16 * sizeof(char16_t));
  if (!utf16_output) return EXIT_FAILURE;
  ...
  size_t expected_utf8 = simdutf_utf8_length_from_utf16le(utf16_output, utf16words);
  char *utf8_output = (char *)malloc(expected_utf8 + 1);
  if (!utf8_output) { free(utf16_output); return EXIT_FAILURE; }
  size_t utf8words = simdutf_convert_utf16le_to_utf8(utf16_output, utf16words, utf8_output);
