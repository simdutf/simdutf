...
#ifndef SIMDUTF_ARM64_SAFE_ZERO_CHECK
  #define SIMDUTF_ARM64_SAFE_ZERO_CHECK 0
#endif
...
// caller.
//
// By default this compiles to two instructions (shrn + fcmp) and, unlike a
// reduction such as vmaxvq_u8, it never moves the value to a general-purpose
// register. Such a transfer has a latency of about 3 cycles on Apple hardware,
// on top of the 3 cycles of the reduction itself.
//
...
// every byte is 0x00 or 0xff. The floating-point comparison is safe under IEEE
// default settings: the only non-zero bit pattern that compares equal to 0.0 is
// -0.0 (0x8000000000000000), and the most significant byte of the narrowed
// value can only be 0x00, 0x0f, 0xf0 or 0xff.
//
// If flush-to-zero is enabled, denormal bit patterns also compare equal to
// 0.0, so a mask that is non-zero only in the low bytes can be reported as
// all-zero. Set SIMDUTF_ARM64_SAFE_ZERO_CHECK to 1 to use a vmaxvq_u32
// reduction instead; it is slower but independent of the floating-point
// environment.
//
...
#if SIMDUTF_ARM64_SAFE_ZERO_CHECK
