...
size_t convert_masked_utf8_to_utf16(const char *input,
  ...
  const vecu8 in = (vecu8)(vec_vsx_ld(0, reinterpret_cast<const uint8_t *>(input)));
  const uint16_t input_utf8_end_of_code_point_mask =
      utf8_end_of_code_point_mask & 0xFFF;
  if(((utf8_end_of_code_point_mask & 0xFFFF) == 0xFFFF)) {
    // We process the data in chunks of 16 bytes.
    vec_vsx_st(vec_mergel(in, vec_splat_u8(0)), 0, reinterpret_cast<vecu8 *>(utf16_output));
    vec_vsx_st(vec_mergeh(in, vec_splat_u8(0)), 0, reinterpret_cast<vecu8 *>(utf16_output + 8));
  ...
  if(((utf8_end_of_code_point_mask & 0xFFFF) == 0xaaaa)) {
    ...
    const vecu16 perm = reinterpret_cast<vecu16>(vec_perm(in, in, sh));
    const vecu16 ascii = vec_and(perm,vec_splat_u16(0x7f));
    const vecu16 highbyte = vec_and(perm, vec_splat_u16(0x1f00));
    const vecu16 composed = vec_or(ascii, vec_srl(highbyte, vec_splat_u16(2)));
    vec_vsx_st(composed, 0, reinterpret_cast<vecu16 *>(utf16_output));
  ...
  if(input_utf8_end_of_code_point_mask == 0x924) {
    ...
    // There is probably a more efficient sequence, but the following might do.
    const vecu8 sh = {2, 1, 0, 127, 5, 4, 3, 127, 8, 7, 6, 127, 11, 10, 9, 127}; // the 127 become 15, we use them to mark unwanted values.
    const vecu32 perm = reinterpret_cast<vecu32>(vec_perm(in, in, sh));
    const vecu32 ascii =
        vec_and(perm, vec_splat_u32(0x7f)); // 7 or 6 bits
    const vecu32 middlebyte =
        vec_and(perm, vec_splat_u32(0x3f00)); // 5 or 6 bits
    const vecu32 middlebyte_shifted = vec_srl(middlebyte, vec_splat_u32(2));
    const vecu32 highbyte =
        vec_and(perm, vec_splat_u32(0x0f0000)); // 4 bits
    const vecu32 highbyte_shifted = vec_srl(highbyte, vec_splat_u32(4));
    const vecu32 composed =
        vec_or(vec_or(ascii, middlebyte_shifted), highbyte_shifted);
    const vecu16 composed_repacked = reinterpret_cast<vecu16>(vec_packsu(composed, composed));
    vec_vsx_st(composed_repacked, 0, reinterpret_cast<vecu16 *>(utf16_output));
    utf16_output += 4;
    return 12;
  }
  ...
  if (idx < 64) {
    ...
    const vecu32 correct =
        vec_srl(vec_and(perm, vec_splat_u32(0x400000)), vec_splat_u32(1));
    middlehighbyte = vec_xor(correct, middlehighbyte);
    const vecu32 middlehighbyte_shifted = vec_srl(middlehighbyte, vec_splat_u32(4));
    const vecu32 highbyte = vec_and(perm, vec_splat_u32(0x07000000));
    const vecu32 highbyte_shifted = vec_srl(highbyte, vec_splat_u32(6));
    const vecu32 composed =
    ...
    const vecu32 hightenbits = vec_srl(composedminus, vec_splat_u32(10));
    ...
    const vecu32 lowtenbitsaddshifted = vec_srl(lowtenbitsadd, vec_splat_u32(16));
