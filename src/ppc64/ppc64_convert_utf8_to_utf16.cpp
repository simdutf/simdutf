...
size_t convert_masked_utf8_to_utf16(const char *input,
  ...
  const __m128i in = (__m128i)(vec_vsx_ld(0, reinterpret_cast<const uint8_t *>(values)));
  const uint16_t input_utf8_end_of_code_point_mask =
      utf8_end_of_code_point_mask & 0xFFF;
  if(((utf8_end_of_code_point_mask & 0xFFFF) == 0xFFFF)) {
    // We process the data in chunks of 16 bytes.
    vec_vsx_st(_mm_cvtepu8_epi16(in), 0, reinterpret_cast<__m128i *>(utf16_output));
    vec_vsx_st(_mm_cvtepu8_epi16(_mm_srli_si128(in,8), 0, reinterpret_cast<__m128i *>(utf16_output + 8));
  ...
  if(((utf8_end_of_code_point_mask & 0xFFFF) == 0xaaaa)) {
    ...
    const __m128i perm = vec_perm(in, in, sh);
    const __m128i ascii = vec_and(perm, _mm_set1_epi16(0x7f));
    const __m128i highbyte = vec_and(perm, _mm_set1_epi16(0x1f00));
    const __m128i composed = vec_or(ascii, _mm_srli_epi16(highbyte, 2));
    vec_vsx_st(composed, 0, reinterpret_cast<__m128i *>(utf16_output));
  ...
  if(input_utf8_end_of_code_point_mask == 0x924) {
    ...
    const __m128i perm = vec_perm(in, in, sh);
    const __m128i ascii =
        vec_and(perm, _mm_set1_epi32(0x7f)); // 7 or 6 bits
    const __m128i middlebyte =
        vec_and(perm, _mm_set1_epi32(0x3f00)); // 5 or 6 bits
    const __m128i middlebyte_shifted = _mm_srli_epi32(middlebyte, 2);
    const __m128i highbyte =
        vec_and(perm, _mm_set1_epi32(0x0f0000)); // 4 bits
    ...
    vec_vsx_st(composed_repacked, 0, reinterpret_cast<__m128i *>(utf16_output));
    utf16_output += 4;
    return 12;
  }
  ...
  if (idx < 64) {
    ...
    const __m128i sh = (__m128i)(vec_vsx_ld(0, reinterpret_cast<const uint8_t *>(tables::utf8_to_utf16::shufutf8[idx])));
    const __m128i perm = vec_perm(in, in, sh);
    const __m128i ascii = vec_and(perm, _mm_set1_epi16(0x7f));
    const __m128i highbyte = vec_and(perm, _mm_set1_epi16(0x1f00));
    const __m128i composed = vec_or(ascii, _mm_srli_epi16(highbyte, 2));
    ...
    const __m128i sh = (__m128i)(vec_vsx_ld(0, reinterpret_cast<const uint8_t *>(tables::utf8_to_utf16::shufutf8[idx])));
    const __m128i perm = vec_perm(in, in, sh);
    const __m128i ascii = vec_and(perm, _mm_set1_epi32(0x7f));
    const __m128i middlebyte = vec_and(perm, _mm_set1_epi32(0x3f00));
    const __m128i middlebyte_shifted = _mm_srli_epi32(middlebyte, 2);
    __m128i middlehighbyte = vec_and(perm, _mm_set1_epi32(0x3f0000));
    // correct for spurious high bit
    const __m128i correct =
        _mm_srli_epi32(vec_and(perm, _mm_set1_epi32(0x400000)), 1);
    middlehighbyte = vec_xor(correct, middlehighbyte);
    const __m128i middlehighbyte_shifted = _mm_srli_epi32(middlehighbyte, 4);
    const __m128i highbyte = vec_and(perm, _mm_set1_epi32(0x07000000));
    ...
    const __m128i lowtenbits =
        vec_and(composedminus, _mm_set1_epi32(0x3ff));
    ...
    vec_vsx_st(composed, 0, reinterpret_cast<__m128i *>(basic_buffer));
    uint32_t surrogate_buffer[4];
    vec_vsx_st(surrogates, 0, reinterpret_cast<__m128i *>(surrogate_buffer));
