...
static void unittest_encoding_translate() {
  ...
  for (uint8_t value = 0; value < 64; value++) {
    ...
    const auto got = lookup_pshufb_improved<false>(in);
  ...
  for (uint8_t value = 0; value < 64; value++) {
    const auto in = vector_u8::splat(value);
    const auto got = lookup_pshufb_improved<true>(in);
...
static void unittest_encoding_expand_6bit_fieds() {
  for (size_t position = 0; position < 10; position++) {
    for (uint64_t value = 0; value < 64; value++) {
      ...
      const auto expanded = expand_6bit_fields(input);
...
static void unittest_implementation() {
  unittest_encoding_translate();
  unittest_encoding_expand_6bit_fieds();
  puts("All OK");
}
