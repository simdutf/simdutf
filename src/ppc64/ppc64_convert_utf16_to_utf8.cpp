...
ppc64_convert_utf16_to_utf8(const char16_t *buf, size_t len,
                            char *utf8_output) {
  ...
  while (end - buf >= std::ptrdiff_t(16 + safety_margin)) {
    ...
    const auto one_or_two_bytes_bytemask = in < int(1 << 11);
    ...
    if (surrogates_bitmask == 0x0000) {
      ...
      */
      ...
      // b0 = [00bb|bbbb|0000|0000]
      const auto b0 = in.shl<2>() & uint16_t(0x3f00);
      // a0 = [0000|0000|0000|aaaa]
      const auto a0 = in.shr<12>();

      // s3 = [11bb|bbbb|1110|aaaa]
      const auto s3 = a0 | b0 | uint16_t(0b1100000011100000);

      const auto m0 = ~as_vector_u16(one_or_two_bytes_bytemask) &
                      uint16_t(0b0100000000000000);
      const auto s4 = s3 ^ m0;
      ...
      if (mask == 0) {
        ...
        const auto shuffle1 = vector_u8(10, 26, 13, 12, 28, 15, 14, 30, -1, -1,
                                        -1, -1, -1, -1, -1, -1);

        const auto utf8_0 =
            shuffle0.lookup_32(as_vector_u8(s4), as_vector_u8(t2));
        const auto utf8_1 =
            shuffle1.lookup_32(as_vector_u8(s4), as_vector_u8(t2));
      ...
      const uint8_t *row0 =
          &simdutf::tables::ppc64_utf16_to_utf8::pack_1_2_3_utf8_bytes[mask0]
                                                                      [0];
      ...
      const uint8_t *row1 =
          &simdutf::tables::ppc64_utf16_to_utf8::pack_1_2_3_utf8_bytes[mask1]
                                                                      [0];
...
/*
  ...
  error. Otherwise, it is the position of the first unproceppc64d byte in buf
  (even if finished). A scalar routing should carry on the conversion of the
  tail if needed.
*/
template <endianness big_endian>
std::pair<result, char *>
ppc64_convert_utf16_to_utf8_with_errors(const char16_t *buf, size_t len,
                                        char *utf8_output) {
