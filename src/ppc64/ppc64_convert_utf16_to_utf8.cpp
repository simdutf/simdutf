...
std::pair<const char16_t*, char*> ppc64_convert_utf16_to_utf8(const char16_t* buf, size_t len, char* utf8_out) {
  ...
  while (buf + 16 <= end) {
    __m128i in = (__m128i)(vec_vsx_ld(0, reinterpret_cast<const uint8_t *>(buf)));
    ...
    if(_mm_testz_si128(in, v_ff80)) { // ASCII fast path!!!!
        __m128i nextin = (__m128i)(vec_vsx_ld(0, reinterpret_cast<const uint8_t *>(buf)+1));
        if(!_mm_testz_si128(nextin, v_ff80)) {
          // 1. pack the bytes
          // obviously suboptimal.
          const __m128i utf8_packed = _mm_packus_epi16(in,in);
          // 2. store (16 bytes)
          vec_vsx_st(utf8_packed, 0, reinterpret_cast<__m128i *>(utf8_output));
          // 3. adjust pointers
          buf += 8;
          utf8_output += 8;
          in = nextin;
    ...
    const __m128i one_byte_bytemask = _mm_cmpeq_epi16(vec_and(in, v_ff80), v_0000);
    const uint16_t one_byte_bitmask = static_cast<uint16_t>(_mm_movemask_epi8(one_byte_bytemask));

    // no bits set above 11th bit
    const __m128i one_or_two_bytes_bytemask = _mm_cmpeq_epi16(vec_and(in, v_f800), v_0000);
    ...
    if (one_or_two_bytes_bitmask == 0xffff) {
          ...
          const __m128i t1 = vec_and(t0, v_1f00);
          // t2 = [0000|0000|00bb|bbbb]
          const __m128i t2 = vec_and(in, v_003f);
          // t3 = [000a|aaaa|00bb|bbbb]
          const __m128i t3 = vec_or(t1, t2);
          // t4 = [110a|aaaa|10bb|bbbb]
          const __m128i t4 = vec_or(t3, v_c080);
          ...
          const __m128i shuffle = (__m128i)(vec_vsx_ld(0, reinterpret_cast<const uint8_t *>(row + 1)));

          const __m128i utf8_packed = vec_perm(utf8_unpacked, utf8_unpacked, shuffle);
    ...
    const __m128i surrogates_bytemask = _mm_cmpeq_epi16(vec_and(in, v_f800), v_d800);
...
#define vec(x) _mm_set1_epi16(static_cast<uint16_t>(x))
        ...
        const __m128i t0 = vec_perm(in, in, dup_even);
        // [bbcc|cccc|bbcc|cccc] => [00cc|cccc|0bcc|cccc]
        const __m128i t1 = vec_and(t0, vec(0b0011111101111111));
        // [00cc|cccc|0bcc|cccc] => [10cc|cccc|0bcc|cccc]
        const __m128i t2 = vec_or (t1, vec(0b1000000000000000));

        // [aaaa|bbbb|bbcc|cccc] =>  [0000|aaaa|bbbb|bbcc]
        const __m128i s0 = _mm_srli_epi16(in, 4);
        // [0000|aaaa|bbbb|bbcc] => [0000|aaaa|bbbb|bb00]
        const __m128i s1 = vec_and(s0, vec(0b0000111111111100));
        ...
        const __m128i s3 = vec_or(s2, vec(0b1100000011100000));
        const __m128i m0 = vec_andc(one_or_two_bytes_bytemask, vec(0b0100000000000000));
        const __m128i s4 = vec_xor(s3, m0);
#undef vec
        ...
        if(mask == 0) {
          ...
          const __m128i utf8_0 = vec_perm(out0, out0, shuffle);
          const __m128i utf8_1 = vec_perm(out1, out1, shuffle);
          vec_vsx_st(utf8_0, 0, reinterpret_cast<__m128i *>(utf8_output));
          utf8_output += 12;
          vec_vsx_st(utf8_1, 0, reinterpret_cast<__m128i *>(utf8_output));
          utf8_output += 12;
          buf += 8;
          continue;
        }
        ...
        const __m128i shuffle0 = (__m128i)(vec_vsx_ld(0, reinterpret_cast<const uint8_t *>(row0 + 1)));
        
        const __m128i utf8_0 = vec_perm(out0, out0, shuffle0);
        ...
        const __m128i shuffle1 = (__m128i)(vec_vsx_ld(0, reinterpret_cast<const uint8_t *>(row1 + 1)));

        const __m128i utf8_1 = vec_perm(out1, out1, shuffle1);
        vec_vsx_st(utf8_0, 0, reinterpret_cast<__m128i *>(utf8_output));
        utf8_output += row0[0];
        vec_vsx_st(utf8_1, 0, reinterpret_cast<__m128i *>(utf8_output));
        utf8_output += row1[0];
