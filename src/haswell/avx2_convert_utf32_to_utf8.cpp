std::pair<const char32_t*, char*> avx2_convert_utf32_to_utf8(const char32_t* buf, size_t len, char* utf8_output) {
  ...
  while (buf + 16 + safety_margin <= end) {
    ...
    __m128i in_16 = _mm_packus_epi32(_mm256_castsi256_si128(in), _mm256_extractf128_si256(in,1));
    
    // Try to apply UTF-16 => UTF-8 routine on 128 bits (../westmere/sse_convert_utf16_to_utf8.cpp)
    
    if(_mm_testz_si128(in_16, v_ff80)) { // ASCII fast path!!!!
      __m256i nextin = _mm256_loadu_si256((__m256i*)buf+1);
      __m128i nextin_16 = _mm_packus_epi32(_mm256_castsi256_si128(nextin),_mm256_extractf128_si256(nextin,1));
