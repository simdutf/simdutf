...
avx2_convert_utf32_to_latin1(const char32_t *buf, size_t len,
  ...
  for (size_t i = 0; i < rounded_len; i += 4 * 8) {
    ...
    __m256i c = _mm256_loadu_si256((__m256i *)(buf + 1 * 8));
    __m256i d = _mm256_loadu_si256((__m256i *)(buf + 1 * 8));
    ...
    b = _mm256_srli_epi32(b, 1 * 8);
    c = _mm256_srli_epi32(c, 2 * 8);
    d = _mm256_srli_epi32(d, 3 * 8);
...
// clang-format off
    ...
    // t0 = [d7 c7 b7 a7|d6 c6 b6 a6|d5 c5 b5 a5|d4 c4 b4 a4|d3 c3 b3 a3|d2 c2
    // b2 a2|d1 c1 b1 a1|d0 c0 b0 a0]
    const __m256i t0 =
        _mm256_or_si256(_mm256_or_si256(a, b), _mm256_or_si256(c, d));
