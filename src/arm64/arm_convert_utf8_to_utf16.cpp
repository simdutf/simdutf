...
size_t convert_masked_utf8_to_utf16(const char *input,
  ...
  if((utf8_end_of_code_point_mask & 0xffff) == 0xffff) {
    ...
    if (!match_system(big_endian)) {
      ascii_first = vreinterpretq_u16_u8(vrev16q_u8(vreinterpretq_u8_u16(ascii_first)));
      ascii_second = vreinterpretq_u16_u8(vrev16q_u8(vreinterpretq_u8_u16(ascii_second)));
    }
  ...
  if((utf8_end_of_code_point_mask & 0xffff) == 0xaaaa) {
    ...
    if (!match_system(big_endian)) composed = vreinterpretq_u16_u8(vrev16q_u8(vreinterpretq_u8_u16(composed)));
  ...
  if(input_utf8_end_of_code_point_mask == 0x924) {
    ...
    if (!match_system(big_endian)) composed= vreinterpret_u16_u8(vrev16_u8(vreinterpret_u8_u16(composed)));
  ...
  if(input_utf8_end_of_code_point_mask == 0x888) {
    ...
    uint8x16_t swap1 = vrev16q_u8(in);
    // Shift left 2 bits
    // cccccc00 dddddd00 xxxxxxxx bbbbbb00
    uint32x4_t shift = vreinterpretq_u32_u8(vshlq_n_u8(swap1, 2));
    // Insert low 2 bits of trail surrogate to magic number for later

    // UTF-8 4b prefix  = -0xF000|0000
    // surrogate offset = -0x0040|0000 (0x10000 << 6)
    // surrogate high   = +0xD800|0000
    // surrogate low    = +0x0000|DC00
    // -------------------------------
    //                  = +0xE7C0|DC00
    // 11011100 00000000 11100111 110000cc
    uint16x8_t magic = vreinterpretq_u16_u32(VSRAQ_N_U32(vmovq_n_u32(0xDC00E7C0), shift, 30));
    // Generate unadjusted trail surrogate minus lowest 2 bits
    // xxxxxxxx xxxxxxxx|11110aaa bbbbbb00
    uint32x4_t trail = VBSLQ_U32(vmovq_n_u32(0x0000FF00), swap1, shift);
    // Generate lead surrogate
    // xxxxcccc ccdddddd|xxxxxxxx xxxxxxxx
    uint32x4_t lead = vsliq_n_u16(swap1, in, 6);
    // Mask out lead
    // 000000cc ccdddddd|xxxxxxxx xxxxxxxx
    uint32x4_t masked = vbicq_u32(lead, vmovq_n_u32(0xFC000000));
    // Blend pairs
    // 000000cc ccdddddd|11110aaa bbbbbb00
    uint16x8_t blend = vreinterpretq_u16_u32(VBSLQ_U32(vmovq_n_u32(0x0000FFFF), trail, masked));
    // Add magic number to correct UTF-8 prefix, surrogate offset, and surrogate prefixes.
    // 110111CC CCDDDDDD|110110AA BBBBBBCC
    uint16x8_t composed = vaddq_u16(blend, magic);
    if (!match_system(big_endian)) composed = vreinterpretq_u16_u8(vrev16q_u8(vreinterpretq_u8_u16(composed)));
    vst1q_u16(reinterpret_cast<uint16_t *>(utf16_output), composed);
  ...
  if (idx < 64) {
    ...
    // This gives us a free shift for highperm.
    // 3 byte:   aaaaxxxx xxxxxxxx
    uint16x4_t highperm = vshrn_n_u32(perm, 4);
    // 1 byte: 00000000 0ccccccc
    // 2 byte: 110bbbbb 10cccccc
    // 3 byte: 10bbbbbb 10cccccc
    uint16x4_t lowperm = vmovn_u32(perm);
    // ascii
    uint16x4_t ascii = vand_u32(lowperm, vmov_n_u16(0x7F));
    // 1 byte: 00000000 00000000
    // 2 byte: 000bbbbb 00000000
    // 3 byte: 000bbbbb 00000000
    uint16x4_t middlebyte = vand_u16(lowperm, vmov_n_u16(0x3f00));
    // Bitwise select the top and bottom bits
    // 1 byte: 00000000 0ccccccc
    // 2 byte: 00000000 00cccccc
    // 3 byte: aaaa0000 00cccccc
    uint16x4_t lowhigh = VBSL_U16(vmov_n_u16(0x0FFF), ascii, highperm);
    // Shift right accumulate the middle byte
    // 1 byte: 00000000 0ccccccc
    // 2 byte: 00000bbb bbcccccc
    // 3 byte: aaaabbbb bbcccccc
    uint16x4_t composed = vsra_n_u16(lowhigh, middlebyte, 2);
    if (!match_system(big_endian)) composed = vreinterpret_u16_u8(vrev16_u8(vreinterpret_u8_u16(composed)));
    vst1_u16(reinterpret_cast<uint16_t*>(utf16_output), composed);
    ...
    uint32x4_t is_pair = vreinterpretq_u32_s32(vcltzq_s32(vreinterpretq_s32_u32(perm)));
    ...
    uint32x4_t ab = VBSLQ_U32(vmovq_n_u32(0xFF000000), perm, vshlq_n_u32(perm, 2));
    ...
    // Number is halfword-swapped from above.
    ...
    // Byte swap
    if (!match_system(big_endian)) {
      selected = vreinterpretq_u32_u8(vrev16q_u8(vreinterpretq_u32_u8(selected)));
    }
    uint32_t buffer[4];
