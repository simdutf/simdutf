  ...
  while (buf + 16 + safety_margin <= end) { //buf is a char32_t pointer, each char32_t has 4 bytes or 32 bits, thus buf + 16 * char_32t = 512 bits = 64 bytes
    ...
    if(_mm_testz_si128(in_16, v_ff80)) {  //if the first two blocks are ASCII
      ...
      __m128i nextin_16 = _mm_packus_epi32(
                                            ...
                                            _mm_and_si128(fourthin, v_7fffffff));//pack into 1 vector, now you have two packed vectors
      if(!_mm_testz_si128(nextin_16, v_ff80)) {  //checks if the second packed vector is ASCII, if not:
        // 1. pack the bytes
        // obviously suboptimal.
        const __m128i utf8_packed = _mm_packus_epi16(in_16,in_16); //creates two copy of the first packed vector
        // 2. store (16 bytes)
        _mm_storeu_si128((__m128i*)utf8_output, utf8_packed); //put them into the output
        // 3. adjust pointers
        // Here we consider the second packed vector
        ...
        in = thirdin;
        nextin = fourthin;
      } else { // if they are ASCII characters
        // 1. pack the bytes
    ...
    // This 
    const __m128i one_byte_bytemask = _mm_cmpeq_epi16( // this takes 16 bits = 2 bytes at a time and compares: 
                                                      _mm_and_si128(in_16, 
                                                                    v_ff80), // the first 9 bits of the first packed vector and
                                                       v_0000
                                                       ); // they should be all zero if they are ASCII. E.g. ASCII in UTF32 is of format 0000 0000 0000 0XXX XXXX
    // _mm_cmpeq_epi16 should now return a 1111 1111 1111 1111 for equals (or ASCII in this case), and 0000 0000 0000 0000 if not for each 16-bit/2-byte units
    ...
    // e.g. it compacts the original 256 bits mask to one convenients 16 bits mask

    // no bits set above 11th bit
    // same concept but with 11 bit this time
    const __m128i one_or_two_bytes_bytemask = _mm_cmpeq_epi16(
                                                              _mm_and_si128(in_16,v_f800), 
                                                              v_0000);
    ...
    if (one_or_two_bytes_bitmask == 0xffff) {
      // case: all words either produce 1 or 2 UTF-8 bytes (at least one produces 2 bytes because at this point we have non-ASCII characters)
      // 1. prepare 2-byte values
      // input 16-bit word : [0000|0aaa|aabb|bbbb] x 8 -- format of UTF-32 unit that translates to 2-byte UTF-8 unit
      ...
      const uint16_t m1 = static_cast<uint16_t>(m0 >> 7);           // m1 = 0000 0000 h0g0 f0e0
      const uint8_t  m2 = static_cast<uint8_t>((m0 | m1) & 0xff);   // m2 =           hdgc fbea
      // 4. pack the bytes
      const uint8_t* row = &simdutf::tables::utf16_to_utf8::pack_1_2_utf8_bytes[m2][0]; //lookup table m2 designates the row, *row represents the start of it
