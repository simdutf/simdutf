...
size_t valid_utf8_to_latin1_avx512(const char *buf, size_t len, char *latin_output) {
    ...
    while (pos + 64 <= len) {
        size_t written = process_valid_block<false>(buf + pos, 64, latin_output, minus64, one, &next_leading, &next_bit6 );
        if (written == 0) {
    ...
    if (pos < len) {
        ...
        size_t written = process_valid_block<true>(buf + pos, remaining, latin_output, minus64, one, &next_leading, &next_bit6);
        if (written == 0) {
            return 0; // Indicates error
        }
        latin_output += written;
