...
simdutf_really_inline size_t process_valid_block(const char *buf, size_t len, char *latin_output,
    ...
    // If all characters are ASCII, simply store and return
    if (nonascii == 0) {
        is_remaining ? _mm512_mask_storeu_epi8((__m512i *)latin_output, load_mask, input) : _mm512_storeu_si512((__m512i *)latin_output, input);
        return len;
    }

    // Extract the leading bytes
    __mmask64 leading = _mm512_cmpge_epu8_mask(input, minus64);

    // Shift and adjust leading bytes and bit6 values for next iteration
    __mmask64 leading_shift = (leading << 1) | *next_leading_ptr;
    *next_leading_ptr = leading >> 63;
    ...
    *next_bit6_ptr = bit6 >> 63;
    ...
    // Compute the characters to retain and compress the output
    __mmask64 retain = ~leading & load_mask;
    __m512i output = _mm512_maskz_compress_epi8(retain, input);
...
size_t valid_utf8_to_latin1_avx512(const char *buf, size_t len, char *latin_output) {
    ...
    while (pos + 64 <= len) {
        size_t written = process_valid_block<false>(buf + pos, 64, latin_output, minus64, one, &next_leading, &next_bit6 , load_mask);
    ...
    if (pos < len) {
        ...
        size_t written = process_valid_block<true>(buf + pos, remaining, latin_output, minus64, one, &next_leading, &next_bit6 , load_mask);
        latin_output += written;
    }
