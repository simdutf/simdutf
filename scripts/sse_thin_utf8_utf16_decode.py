...
def main():
  ...
  print("/* memory usage : " + str(len(allshuf)*16.0/1024) + " kB */")
  total_memory += len(allshuf)*16
  ...
  for t in easycase12sorted + easycase123sorted + easycase1234sorted:
      ...
      c = c + 1
  arrg=[]
  consumed=[0 for i in range(len(allshuf))]
  for x in range(1<<12):
    sizes = compute_code_point_size(x)
    if(easy_case12(sizes)):
        z1 = grab_easy_case12_code_point_size(sizes)
        ...
        arrg.append(0)
  print("/* memory usage : " + str(len(consumed)) + " B */")
  total_memory += len(consumed)
  print("const uint8_t consumed["+str(len(consumed))+"] = ")
  print(cpp_array_initializer(consumed), end=";\n")
  print("/* memory usage : " + str(len(arrg)/1024) + " kB */")
  print("const uint8_t utf8index["+str(len(arrg))+"] = ")
  print(cpp_array_initializer(arrg), end=";\n")
  total_memory += len(arrg)
  print("/* total memory usage (all tables) : " + str(total_memory/1024) + " kB */")
