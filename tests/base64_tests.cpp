...
TEST(binary_length_from_base64_url_variant) {
  // Test with URL-safe base64 variant
  // URL-safe uses '-' instead of '+' and '_' instead of '/'
  std::string url_input = "YWJj"; // Same as standard for this input
  ASSERT_EQUAL(simdutf::binary_length_from_base64(
                   url_input.data(), url_input.size(), simdutf::base64_url),
               3);
  ...
  ASSERT_EQUAL(simdutf::binary_length_from_base64(
                   url_input2.data(), url_input2.size(), simdutf::base64_url),
               4);
}
