...
TEST(valid_utf16le_roundtrip) {
  ...
  size_t expected_len = simdutf::utf8_length_from_utf16le(input.data(), input.size());
  std::vector<char> expected(expected_len);
  size_t written_expected =
      implementation.convert_utf16le_to_utf8(input.data(), input.size(), expected.data());
...
TEST_LOOP(length_consistency_le) {
  ...
  simdutf::result len_result =
      simdutf::utf8_length_from_utf16le_with_replacement(
...
TEST_LOOP(matches_well_formed_approach_le) {
  ...
  size_t written_expected =
      simdutf::convert_utf16le_to_utf8(well_formed.data(), length, expected.data());
  ...
  ASSERT_EQUAL(written_expected, written_actual);
  ASSERT_TRUE(
      std::memcmp(expected.data(), actual.data(), written_actual) == 0);
}
