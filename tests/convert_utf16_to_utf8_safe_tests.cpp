...
TEST(safe_with_details_exact_fit_three_byte) {
  constexpr size_t n = 17;
  std::vector<char16_t> input(n, char16_t(0x20ac));
  std::vector<char> output(3 * n);
  const simdutf::full_result result =
      simdutf::convert_utf16_to_utf8_safe_with_details(
          input.data(), input.size(), output.data(), output.size());
  ASSERT_EQUAL(result.error, simdutf::error_code::SUCCESS);
  ASSERT_EQUAL(result.input_count, n);
  ASSERT_EQUAL(result.output_count, 3 * n);
}
