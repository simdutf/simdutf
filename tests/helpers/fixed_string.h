...
struct TmpStringLiteral {
   ...
   */
  constexpr TmpStringLiteral(CharType const (&str)[N]) {
 ...
 */
template <detail::valid_chartype CharType, std::size_t N,
          std::endian endianess_ = std::endian::native>
struct CTString {
  /// the string, not null terminated
  CharType storage[N];

  using type = CharType;
  static constexpr std::endian endianess = endianess_;
...
struct is_CTString<CTString<CharType, N, endianess>> : std::true_type {};
...
concept utf16le_ctstring =
    (detail::is_CTString<std::decay_t<T>>::value &&
     std::is_same_v<typename std::decay_t<T>::type, char16_t> &&
     T::endianess == std::endian::little);

template <class T>
concept utf16be_ctstring =
    (detail::is_CTString<std::decay_t<T>>::value &&
     std::is_same_v<typename std::decay_t<T>::type, char16_t> &&
     T::endianess == std::endian::big);
...
/// converts a CTString into a std::string
/*constexpr*/ std::string to_stdstring(latin1_ctstring auto &&input) {
  return std::string(std::begin(input), std::end(input));
}

/*constexpr*/ std::string to_stdstring(utf8_ctstring auto &&input) {
  return std::string(std::begin(input), std::end(input));
}
