...
template <typename CharType, std::size_t N, std::endian endianess>
constexpr auto to_latin1(const CTString<CharType, N, endianess> &input) {
  CTString<char, N> tmp;
  std::size_t ret;
...
namespace detail {
template <std::endian output_endianess, typename CharType, std::size_t N,
          std::endian input_endianess>
constexpr auto
to_utf16_impl(const CTString<CharType, N, input_endianess> &input) {

  if constexpr (std::is_same_v<CharType, char16_t>) {
    if constexpr (output_endianess == input_endianess) {
      // no-op
      return input;
    } else {
      // byteswap
      CTString<CharType, N, output_endianess> output;
...
template <typename CharType, std::size_t N, std::endian endianess>
constexpr auto to_utf16(const CTString<CharType, N, endianess> &input) {
  return detail::to_utf16_impl<std::endian::native>(input);
}

/// converts valid input to utf16le
template <typename CharType, std::size_t N, std::endian endianess>
constexpr auto to_utf16le(const CTString<CharType, N, endianess> &input) {
  return detail::to_utf16_impl<std::endian::little>(input);
}

/// converts valid input to utf16be
template <typename CharType, std::size_t N, std::endian endianess>
constexpr auto to_utf16be(const CTString<CharType, N, endianess> &input) {
...
template <std::endian target_endianess, bool with_errors, auto input>
constexpr auto utf8_to_utf16() {
  using namespace simdutf::tests::helpers;
  constexpr auto Nout = simdutf::utf16_length_from_utf8(input);
  CTString<char16_t, Nout, target_endianess> tmp{};
  std::size_t N;
  if constexpr (target_endianess == std::endian::little) {
...
template <std::endian target_endianess, auto input>
...
template <auto input> constexpr auto utf8_length_from_utf16_helper() {
  if constexpr (decltype(input)::endianess == std::endian::little) {
