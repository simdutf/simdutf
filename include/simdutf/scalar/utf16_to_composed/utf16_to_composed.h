...
size_t normalize(const char16_t *input, size_t length, char16_t *out) {
  ...
  while (p < length) {
    ...
    auto previous_starter_pos_result = normalization::rfind_starter<
        normalization::utf16_normalization_traits<big_endian>>(input, p);
    ...
    const char16_t *next_irrelevant_starter_ptr =
        normalization::find_first_stable<
            normalization::utf16_normalization_traits<big_endian>, form>(
    ...
    size_t new_length = normalization::compose_canonical<
        normalization::utf16_normalization_traits<big_endian>, form>(
        normalized_out, normalized_length);
...
size_t normalize_with_context(const char16_t *input, const char16_t *input_base,
  ...
  const char16_t *prev_starter_ptr = normalization::find_last_stable<
      normalization::utf16_normalization_traits<big_endian>, form>(
      input_base, scanned_length);
  size_t prev_starter = (prev_starter_ptr == input_base + scanned_length)
                            ? 0
                            : size_t(prev_starter_ptr - input_base);
  const char16_t *next_starter_ptr = normalization::find_first_stable<
      normalization::utf16_normalization_traits<big_endian>, form>(
