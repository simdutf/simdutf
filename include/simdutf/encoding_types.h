 ...
 * @return     the corresponding std::text_encoding, or an empty
 *             std::text_encoding if the encoding is unspecified or unknown
 */
 ...
 */
constexpr encoding_type from_std_encoding(const std::text_encoding &enc) noexcept {
  switch (enc.mib()) {
 ...
 *
 * When the input is std::text_encoding::id::UTF16 or UTF32 (without LE/BE suffix),
 * this returns the native-endian simdutf variant.
 *
 * @param enc  the std::text_encoding
 * @return     the corresponding simdutf encoding type, or
 *             encoding_type::unspecified if the encoding is not supported
 */
constexpr encoding_type from_std_encoding_native(const std::text_encoding &enc) noexcept {
  switch (enc.mib()) {
  case std::text_encoding::id::UTF8:
    return UTF8;
