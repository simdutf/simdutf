...
utf16fix_block(char16_t *out, const char16_t *in, bool in_place)
{
	...
	lookback = _mm256_loadu_si256((const void *)(in - 1));
	block = _mm256_loadu_si256((const void *)in);
	...
	if (!_mm256_testz_si256(illseq, illseq)) {
		...
		lb_illseq_shifted = _mm256_or_si256(
			_mm256_bsrli_epi128(lb_illseq, 2),
			_mm256_zextsi128_si256(_mm_bslli_si128(_mm256_extracti128_si256(lb_illseq, 1), 14)));
		block_illseq = _mm256_or_si256(
			_mm256_andnot_si256(lb_is_high, block_is_low),
			lb_illseq_shifted);
		...
		_mm256_storeu_si256((void *)out, block);
	} else if (!in_place)
		_mm256_storeu_si256((void *)out, block);
}

void
utf16fix_avx(char16_t *out, const char16_t *in, size_t n)
{
	size_t i;
